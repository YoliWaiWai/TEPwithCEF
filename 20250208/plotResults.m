% 创建绘图函数
function plotResults(year,Hours,cost_ope,s_sum_N_g,s_sum_N_g_ccs,s_sum_N_g_gas,s_g_exist,s_pd_shed,P_load,s_I_lines,s_x_gen_coal_1,s_x_gen_coal_2,s_x_gen_coal_3,s_x_gen_coal_4,s_x_gen_ccs_1,s_x_gen_ccs_2,s_x_gen_ccs_3,s_x_gen_ccs_4,s_x_gen_gas_1,s_x_gen_gas_2,I,J,l_E)
    N = size(P_load, 1);
% 根据选择的年份绘制相应的图形
    subplot(2, 2, 1); % Change subplot to 2 rows and 2 columns
    % Plot yearly planning results
    sum_type_g = sum(s_sum_N_g, 1);
    sum_type_g_ccs = sum(s_sum_N_g_ccs, 1);
    sum_type_g_gas = sum(s_sum_N_g_gas, 1);
    sum_g_exist = sum(s_g_exist, 1);
    sum_s_pd_shed = sum(s_pd_shed);
    sum_type_g_all = [sum_g_exist; sum_type_g;sum_type_g_ccs;sum_type_g_gas;sum_s_pd_shed];
