...
x_gas_max = [4,5];
p_max = mpc.branch(:,RATE_A)*4/Sbase;       %线路传输功率上限
...
ylabel('功率/100MW');
title('净负荷初始数据');
growth_rate = (1.06)^5; % 6% growth rate per year
for year = 2:Years
...
% for i=1:N
    ...
    %机组发电功率上限
    for t = 1:24
        Cons = [Cons, I_gen_coal_1(:,:,:) .* g_min_all(1) <= g_coal_1(:,:,t,:)];
        Cons = [Cons, g_coal_1(:,:,t,:) <= I_gen_coal_1(:,:,:) .* g_max_all(1)];
        Cons = [Cons, I_gen_coal_2(:,:,:) .* g_min_all(2) <= g_coal_2(:,:,t,:)];
        Cons = [Cons, g_coal_2(:,:,t,:) <= I_gen_coal_2(:,:,:) .* g_max_all(2)];
        Cons = [Cons, I_gen_coal_3(:,:,:) .* g_min_all(3) <= g_coal_3(:,:,t,:)];
        Cons = [Cons, g_coal_3(:,:,t,:) <= I_gen_coal_3(:,:,:) .* g_max_all(3)];
        Cons = [Cons, I_gen_coal_4(:,:,:) .* g_min_all(4) <= g_coal_4(:,:,t,:)];
        Cons = [Cons, g_coal_4(:,:,t,:) <= I_gen_coal_4(:,:,:) .* g_max_all(4)];
        % 碳捕集电厂【总功率】的上限就是火电机组的上下限
        % 碳捕集电厂总功率 未建设时为0
        Cons = [Cons, I_gen_ccs_1(:,:,:) .* g_min_all(1) <= g_ccs_1(:,:,t,:)];
        Cons = [Cons, g_ccs_1(:,:,t,:) <= I_gen_ccs_1(:,:,:) .* g_max_all(1)];
        Cons = [Cons, I_gen_ccs_2(:,:,:) .* g_min_all(2) <= g_ccs_2(:,:,t,:)];
        Cons = [Cons, g_ccs_2(:,:,t,:) <= I_gen_ccs_2(:,:,:) .* g_max_all(2)];
        Cons = [Cons, I_gen_ccs_3(:,:,:) .* g_min_all(3) <= g_ccs_3(:,:,t,:)];
        Cons = [Cons, g_ccs_3(:,:,t,:) <= I_gen_ccs_3(:,:,:) .* g_max_all(3)];
        Cons = [Cons, I_gen_ccs_4(:,:,:) .* g_min_all(4) <= g_ccs_4(:,:,t,:)];
        Cons = [Cons, g_ccs_4(:,:,t,:) <= I_gen_ccs_4(:,:,:) .* g_max_all(4)];
        Cons = [Cons, I_gen_gas_1(:,:,:) .* g_min_gas(1) <= g_gas_1(:,:,t,:)];
        Cons = [Cons, g_gas_1(:,:,t,:) <= I_gen_gas_1(:,:,:) .* g_max_gas(1)];
        Cons = [Cons, I_gen_gas_2(:,:,:) .* g_min_gas(2) <= g_gas_2(:,:,t,:)];
        Cons = [Cons, g_gas_2(:,:,t,:) <= I_gen_gas_2(:,:,:) .* g_max_gas(2)];

        % 碳捕集机组能量和二氧化碳流动关系
        Cons = [Cons,g_ccs_1(:,:,t,:) == g_ccs_N_1(:,:,t,:)  + lamda_a * E_ccs_ab_1(:,:,t,:) + lamda_dc * E_ccs_de_1(:,:,t,:) + reshape(P_D1*I_gen_ccs_1(:,:,:), [30, 4, 1, 3])]; % 机组输出总功率
        Cons = [Cons,g_ccs_2(:,:,t,:) == g_ccs_N_2(:,:,t,:)  + lamda_a * E_ccs_ab_2(:,:,t,:) + lamda_dc * E_ccs_de_2(:,:,t,:) + reshape(P_D1*I_gen_ccs_2(:,:,:), [30, 5, 1, 3])];
        Cons = [Cons,g_ccs_3(:,:,t,:) == g_ccs_N_3(:,:,t,:)  + lamda_a * E_ccs_ab_3(:,:,t,:) + lamda_dc * E_ccs_de_3(:,:,t,:) + reshape(P_D1*I_gen_ccs_3(:,:,:), [30, 1, 1, 3])];
        Cons = [Cons,g_ccs_4(:,:,t,:) == g_ccs_N_4(:,:,t,:)  + lamda_a * E_ccs_ab_4(:,:,t,:) + lamda_dc * E_ccs_de_4(:,:,t,:) + reshape(P_D1*I_gen_ccs_4(:,:,:), [30, 3, 1, 3])];
        Cons = [Cons, E_ccs_de_1(:,:,t,:) <= M * I_gen_ccs_1(:,:,:)];
        Cons = [Cons, E_ccs_de_2(:,:,t,:) <= M * I_gen_ccs_2(:,:,:)];
        Cons = [Cons, E_ccs_de_3(:,:,t,:) <= M * I_gen_ccs_3(:,:,:)];
        Cons = [Cons, E_ccs_de_4(:,:,t,:) <= M * I_gen_ccs_4(:,:,:)];
    end
% end
...
%     Cons = [Cons,E_ccs_ab_1 == E_beta * delta_1 .* E_ccs_1];
...
Cons = [Cons, E_beta * (1 - delta_xz ) * E_ccs_1 <= E_ccs_ab_1];
Cons = [Cons, E_ccs_ab_1 <= E_beta * delta_xz * E_ccs_1]; %烟气分流比限值约束直接写成可吸收的二氧化碳上下限
Cons = [Cons, E_beta * (1 - delta_xz ) * E_ccs_2 <= E_ccs_ab_2];
Cons = [Cons, E_ccs_ab_2 <= E_beta * delta_xz * E_ccs_2];
Cons = [Cons, E_beta * (1 - delta_xz ) * E_ccs_3 <= E_ccs_ab_3];
Cons = [Cons, E_ccs_ab_3 <= E_beta * delta_xz * E_ccs_3];
Cons = [Cons, E_beta * (1 - delta_xz ) * E_ccs_4 <= E_ccs_ab_4];
Cons = [Cons, E_ccs_ab_4 <= E_beta * delta_xz * E_ccs_4];
Cons = [Cons, 0 <= E_ccs_de_1];
Cons = [Cons, 0 <= E_ccs_de_2];
Cons = [Cons, 0 <= E_ccs_de_3];
Cons = [Cons, 0 <= E_ccs_de_4];
toc
...
tic
Cons = [Cons,V_ab_1 == K_VE * E_ccs_ab_1]; % 电厂i的溶液存储器t时刻释放二氧化碳所需的溶液体积 注意单位换算
Cons = [Cons,V_ab_2 == K_VE * E_ccs_ab_2];
Cons = [Cons,V_ab_3 == K_VE * E_ccs_ab_3];
Cons = [Cons,V_ab_4 == K_VE * E_ccs_ab_4];
Cons = [Cons,V_de_1 == K_VE * E_ccs_de_1];
Cons = [Cons,V_de_2 == K_VE * E_ccs_de_2];
Cons = [Cons,V_de_3 == K_VE * E_ccs_de_3];
Cons = [Cons,V_de_4 == K_VE * E_ccs_de_4];
...
for h = 1:Hours
    for i = 1:h
        Cons = [Cons,V_R_1(:,:,h,:) == V_R0  + sum(V_ab_1(:,:,1:h,:),3) - sum(V_de_1(:,:,1:h,:),3)]; % 富液变化
        Cons = [Cons,V_R_2(:,:,h,:) == V_R0  + sum(V_ab_2(:,:,1:h,:),3) - sum(V_de_2(:,:,1:h,:),3)];
        Cons = [Cons,V_R_3(:,:,h,:) == V_R0  + sum(V_ab_3(:,:,1:h,:),3) - sum(V_de_3(:,:,1:h,:),3)];
        Cons = [Cons,V_R_4(:,:,h,:) == V_R0  + sum(V_ab_4(:,:,1:h,:),3) - sum(V_de_4(:,:,1:h,:),3)];
        Cons = [Cons,V_L_1(:,:,h,:) == V_L0  + sum(V_de_1(:,:,1:h,:),3) - sum(V_ab_1(:,:,1:h,:),3)]; % 贫液变化
        Cons = [Cons,V_L_2(:,:,h,:) == V_L0  + sum(V_de_2(:,:,1:h,:),3) - sum(V_ab_2(:,:,1:h,:),3)];
        Cons = [Cons,V_L_3(:,:,h,:) == V_L0  + sum(V_de_3(:,:,1:h,:),3) - sum(V_ab_3(:,:,1:h,:),3)];
        Cons = [Cons,V_L_4(:,:,h,:) == V_L0  + sum(V_de_4(:,:,1:h,:),3) - sum(V_ab_4(:,:,1:h,:),3)];
    end
end
for t=1:23
    Cons = [Cons,-rate_max * P_yita <= V_R_1(:,:,t+1,:) - V_R_1(:,:,t,:) <= rate_max * P_yita]; % 流速
    Cons = [Cons,-rate_max * P_yita <= V_R_2(:,:,t+1,:) - V_R_2(:,:,t,:) <= rate_max * P_yita];
    Cons = [Cons,-rate_max * P_yita <= V_R_3(:,:,t+1,:) - V_R_3(:,:,t,:) <= rate_max * P_yita];
    Cons = [Cons,-rate_max * P_yita <= V_R_4(:,:,t+1,:) - V_R_4(:,:,t,:) <= rate_max * P_yita];
end
toc
...
for year = 1:Years
    ...
    for t = year:Years
        %% 常规燃煤机组
        Obj_inv = Obj_inv + A_gen(1) / (1 + r)^(t - year) * sum(sum(x_gen_coal_1(:, :, year)));
        Obj_inv = Obj_inv + A_gen(2) / (1 + r)^(t - year) * sum(sum(x_gen_coal_2(:, :, year)));
        Obj_inv = Obj_inv + A_gen(3) / (1 + r)^(t - year) * sum(sum(x_gen_coal_3(:, :, year)));
        Obj_inv = Obj_inv + A_gen(4) / (1 + r)^(t - year) * sum(sum(x_gen_coal_4(:, :, year)));
        %% 碳捕集机组
        Obj_inv = Obj_inv + A_ccs(1) / (1 + r)^(t - year) * sum(sum(x_gen_ccs_1(:, :, year)));
        Obj_inv = Obj_inv + A_ccs(2) / (1 + r)^(t - year) * sum(sum(x_gen_ccs_2(:, :, year)));
        Obj_inv = Obj_inv + A_ccs(3) / (1 + r)^(t - year) * sum(sum(x_gen_ccs_3(:, :, year)));
        Obj_inv = Obj_inv + A_ccs(4) / (1 + r)^(t - year) * sum(sum(x_gen_ccs_4(:, :, year)));
        %% 燃气机组
        Obj_inv = Obj_inv + A_gas(1) / (1 + r)^(t - year) * sum(sum(x_gen_gas_1(:, :, year)));
        Obj_inv = Obj_inv + A_gas(2) / (1 + r)^(t - year) * sum(sum(x_gen_gas_2(:, :, year)));
        %% 改造
        Obj_inv = Obj_inv + A_gen_trans(1) / (1 + r)^(t - year) * sum(x_trans_gexist(:,year));
    end
end
toc
...
for t = 1:Hours
    for y = 1:Years
        Obj_ope_total = Obj_ope_total + (M * sum(pd_shed(:,t,y)) )*365*5;%切负荷成本 和 原有机组发电成本
        C_q1(t,y) = K_q * ((P_predict23(t) - g_exist_w1(1, t, y)) + (P_predict27(t) - g_exist_w2(1, t, y))); % 弃风惩罚成本
        for i = 1:length(gen_node)
            node = gen_node(i);
            Obj_ope_total = Obj_ope_total + (1 - I_trans_gexist(i, year))* cost(1).* g_exist_c(node,t,y)*365*5 + ...
                I_trans_gexist(i, year) * cost_ccs(1).* g_exist_c(node,t,y)*365*5;%原有燃煤机组考虑是否改造后的发电成本
        end
        for i = 1:4
            Obj_ope_total = Obj_ope_total + sum(sum(cost(i).*sum_type_g(:,i,t,y)))*365*5;%sum_type_g单位：100兆瓦时 cost单位：每100MW费用 总单位就是元
            Obj_ope_total = Obj_ope_total + sum(sum(cost(i).*sum_type_g_ccs(:,i,t,y)))*365*5;
        end
        for i = 1:2
            Obj_ope_total = Obj_ope_total + sum(sum(cost_gas(i).*sum_type_g_gas(:,i,t,y)))*365*5;
        end
    end
end
toc
...
for y = 1:Years
    for i = 1:length(gen_node)
        ...
        cost_carbon_gexist = cost_carbon_gexist + sum(g_exist_c(node,:,y)) * ((1 - I_trans_gexist(i, y))* cei(1)  + ...
            I_trans_gexist(i, y) * cei_ccs(1) - carbon_quota(1,y)) * carbon_tax(y) * 365*5;%原有燃煤机组考虑是否改造后的碳成本
    end
    for i = 1:4
...
Obj_carbon_coal = sum(sum(cost_carbon_coal))* 365*5;
Obj_carbon_ccs = sum(sum(cost_carbon_ccs))* 365*5;
Obj_carbon_gas = sum(sum(cost_carbon_gas))* 365*5;
Obj_carbon =  Obj_carbon_coal + Obj_carbon_ccs + Obj_carbon_gas + cost_carbon_gexist;
Obj_q = sum(sum(C_q1 * 365*5));
toc
display('**Part III 结束**')
    ...
    for i = 1:N
        for t = year:Years
            ...
            for j = 1:size(x_gen_coal_1, 2)
                Obj_inv_coal = Obj_inv_coal + A_gen(1) * s_x_gen_coal_1(i, j, year) / (1 + r)^(t - year);
            end
            for j = 1:size(x_gen_coal_2, 2)
                Obj_inv_coal = Obj_inv_coal + A_gen(2) * s_x_gen_coal_2(i, j, year) / (1 + r)^(t - year);
            end
            for j = 1:size(x_gen_coal_3, 2)
                Obj_inv_coal = Obj_inv_coal + A_gen(3) * s_x_gen_coal_3(i, j, year) / (1 + r)^(t - year);
            end
            for j = 1:size(x_gen_coal_4, 2)
                Obj_inv_coal = Obj_inv_coal + A_gen(4) * s_x_gen_coal_4(i, j, year) / (1 + r)^(t - year);
            end
            %碳捕集机组
            for j = 1:size(x_gen_coal_1, 2)
                Obj_inv_ccs = Obj_inv_ccs+ A_ccs(1) * s_x_gen_ccs_1(i, j, year) / (1 + r)^(t - year);
            end
            for j = 1:size(x_gen_coal_2, 2)
                Obj_inv_ccs = Obj_inv_ccs + A_ccs(2) * s_x_gen_ccs_2(i, j, year) / (1 + r)^(t - year);
            end
            for j = 1:size(x_gen_coal_3, 2)
                Obj_inv_ccs = Obj_inv_ccs + A_ccs(3) * s_x_gen_ccs_3(i, j, year) / (1 + r)^(t - year);
            end
            for j = 1:size(x_gen_coal_4, 2)
                Obj_inv_ccs = Obj_inv_ccs + A_ccs(4) * s_x_gen_ccs_4(i, j, year) / (1 + r)^(t - year);
            end
            %燃气机组
            for j = 1:size(x_gen_gas_1, 2)
                Obj_inv_gas = Obj_inv_gas + A_gas(1) * s_x_gen_gas_1(i, j, year) / (1 + r)^(t - year);
            end
            for j = 1:size(x_gen_gas_2, 2)
                Obj_inv_gas = Obj_inv_gas + A_gas(2) * s_x_gen_gas_2(i, j, year) / (1 + r)^(t - year);
            end
        end
    end
end
...
for t = 1:Hours
    for y = 1:Years
        ...
        for i = 1:4
            cost_ope(2,t,y) = cost_ope(2,t,y) + sum(sum(cost(i).*s_sum_type_g(:,i,t,y)))*365*5;
            cost_ope(3,t,y) = cost_ope(3,t,y) + sum(sum(cost_ccs(i).*s_sum_type_g_ccs(:,i,t,y)))*365*5;
        end
        for i = 1:2
            cost_ope(4,t,y) = cost_ope(4,t,y) + sum(sum(cost_gas(i).*s_sum_type_g_gas(:,i,t,y)))*365*5;
        end
    end
end
...
for y = 1:Years
    for i = 1:length(gen_node)
        node = gen_node(i);
        carbon_emission_gexist(i,y) = (sum(s_g_exist_c(node,:,y)) * ((1 - s_I_trans_gexist(i, y))* cei(1)  + s_I_trans_gexist(i, y) * cei_ccs(1)))*365*5;
        cost_carbon_gexist_years(i,y) = sum(s_g_exist_c(node,:,y)) * ((1 - s_I_trans_gexist(i, y))*cei(1)  + s_I_trans_gexist(i, y) * cei_ccs(1) - carbon_quota(1,y)) * carbon_tax(y)*365*5;%原有燃煤机组考虑是否改造后
    end
end
%总碳排
carbon_emission_coal = sum(sum(s_sum_coal .* cei'))*365*5;
carbon_emission_ccs = sum(sum(sum(sum(s_E_ccs_NET_1,2)+sum(s_E_ccs_NET_2,2)+sum(s_E_ccs_NET_3,2)+sum(s_E_ccs_NET_4,2))))*365*5;
carbon_emission_gas = sum(sum(s_sum_gas .* cei_gas'))*365*5;
...
% 创建按钮回调函数
set(year1Button, 'Callback', @(src, event) plotResults(1,Hours,cost_ope,s_sum_N_g,s_sum_N_g_ccs,s_sum_N_g_gas,s_g_exist,s_pd_shed,P_load,s_I_lines,s_x_gen_coal_1,s_x_gen_coal_2,s_x_gen_coal_3,s_x_gen_coal_4,s_x_gen_ccs_1,s_x_gen_ccs_2,s_x_gen_ccs_3,s_x_gen_ccs_4,s_x_gen_gas_1,s_x_gen_gas_2,I,J,l_E));
set(year2Button, 'Callback', @(src, event) plotResults(2,Hours,cost_ope,s_sum_N_g,s_sum_N_g_ccs,s_sum_N_g_gas,s_g_exist,s_pd_shed,P_load,s_I_lines,s_x_gen_coal_1,s_x_gen_coal_2,s_x_gen_coal_3,s_x_gen_coal_4,s_x_gen_ccs_1,s_x_gen_ccs_2,s_x_gen_ccs_3,s_x_gen_ccs_4,s_x_gen_gas_1,s_x_gen_gas_2,I,J,l_E));
set(year3Button, 'Callback', @(src, event) plotResults(3,Hours,cost_ope,s_sum_N_g,s_sum_N_g_ccs,s_sum_N_g_gas,s_g_exist,s_pd_shed,P_load,s_I_lines,s_x_gen_coal_1,s_x_gen_coal_2,s_x_gen_coal_3,s_x_gen_coal_4,s_x_gen_ccs_1,s_x_gen_ccs_2,s_x_gen_ccs_3,s_x_gen_ccs_4,s_x_gen_gas_1,s_x_gen_gas_2,I,J,l_E));