...
Cons = [Cons,g_min_all(1) <= g_exist_c(gen_node,:,:)];
Cons = [Cons,g_exist_c(gen_node,:,:)<= g_max_all(1)] ;
...
for t = 1:Hours
    for y = 1:Years
            Obj_ope_total = Obj_ope_total + (M * sum(pd_shed(:,t,y)) + sum(cost(1).*g_exist_c(:,t,y)))*365;%切负荷成本 和 原有机组发电成本
...
Obj_carbon_coal = sum(sum(cost_carbon_coal));
Obj_carbon_ccs = sum(sum(cost_carbon_ccs));
Obj_carbon_gas = sum(sum(cost_carbon_gas));
