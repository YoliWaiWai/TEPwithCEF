...
g_min_all = [0.55,0.55,0.4,0.4] .* g_max_all;
g_min_ccs = g_min_all * 0.8;%碳捕集电厂灵活性运行
x_coal_max = [4,14,1,3];
...
carbon_tax = [108 114 119 125 132];         % 碳税﻿ 单位：元/tCO2
A_gen = c_gen * annuity_factor;
A_ccs = c_gen_ccs * annuity_factor;
p_max = mpc.branch(:,RATE_A)*5/Sbase;       %线路传输功率上限【原有线路传输功率太小了】
...
System_demand = xlsread('gtepuc.xlsx',2,'B3:B26')/1000;
P_load(:,:,1)=(System_demand .* pd')';
...
%% Cons2: 机组发电功率约束
% 原有机组
Cons = [Cons, g_exist(gen_c,:,:) == 0];%初始条件：节点1有一台1型机组
...
% 创建年份选择按钮
year1Button = uicontrol(yearGroup, 'Style', 'radiobutton', 'String', '第一年', 'Position', [20 20 100 30]);
year2Button = uicontrol(yearGroup, 'Style', 'radiobutton', 'String', '第二年', 'Position', [140 20 100 30]);
year3Button = uicontrol(yearGroup, 'Style', 'radiobutton', 'String', '第三年', 'Position', [260 20 100 30]);
year4Button = uicontrol(yearGroup, 'Style', 'radiobutton', 'String', '第四年', 'Position', [380 20 100 30]);
year5Button = uicontrol(yearGroup, 'Style', 'radiobutton', 'String', '第五年', 'Position', [500 20 100 30]);

% 创建按钮回调函数
set(year1Button, 'Callback', @(src, event) plotResults(1,Hours,s_Obj_ope,s_sum_type_g,s_sum_type_g_ccs,s_g_exist,s_pd_shed,P_load,s_I_lines,s_x_gen_coal_1,s_x_gen_coal_2,s_x_gen_coal_3,s_x_gen_coal_4,I,J,l_E));
set(year2Button, 'Callback', @(src, event) plotResults(2,Hours,s_Obj_ope,s_sum_type_g,s_sum_type_g_ccs,s_g_exist,s_pd_shed,P_load,s_I_lines,s_x_gen_coal_1,s_x_gen_coal_2,s_x_gen_coal_3,s_x_gen_coal_4,I,J,l_E));
set(year3Button, 'Callback', @(src, event) plotResults(3,Hours,s_Obj_ope,s_sum_type_g,s_sum_type_g_ccs,s_g_exist,s_pd_shed,P_load,s_I_lines,s_x_gen_coal_1,s_x_gen_coal_2,s_x_gen_coal_3,s_x_gen_coal_4,I,J,l_E));
set(year4Button, 'Callback', @(src, event) plotResults(4,Hours,s_Obj_ope,s_sum_type_g,s_sum_type_g_ccs,s_g_exist,s_pd_shed,P_load,s_I_lines,s_x_gen_coal_1,s_x_gen_coal_2,s_x_gen_coal_3,s_x_gen_coal_4,I,J,l_E));
set(year5Button, 'Callback', @(src, event) plotResults(5,Hours,s_Obj_ope,s_sum_type_g,s_sum_type_g_ccs,s_g_exist,s_pd_shed,P_load,s_I_lines,s_x_gen_coal_1,s_x_gen_coal_2,s_x_gen_coal_3,s_x_gen_coal_4,I,J,l_E));