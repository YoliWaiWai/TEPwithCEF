...
period_length = 3;
Hours = 24*4; % Total number of hours
days_season = 365/4;
...
%c_gen_ccs = 0 * c_gen;
c_gen_ccs = 1.3 * c_gen;        % 新建ccs成本
c_gen_gas = [5.94,6.60]' * 1e8;
c_gen_trans = 0.35 * c_gen;
...
carbon_tax =     [95   105   115   125   135   145   155   165]*0.8;         % 碳税﻿ 单位：元/tCO2
...
carbon_quota  = [0.78,0.77,0.76,0.75,0.74;0.79,0.78,0.77,0.76,0.75]* 1e2;
carbon_quota_gas = [0.3305 0.3240  0.3145  0.3098  0.3067]* 1e2;%燃煤机组碳排放基准值
Carbon_dioxide_price = 210;%出售二氧化碳价格
%生成一组24h负荷需求数据
pd = mpc.bus(:,PD)/Sbase; %负荷需求标幺值
pd_total = sum(pd);
System_demand = [330, 317, 311, 309, 315, 336, 358, 411, 478, 669, 723, 710, 700, 724, 730, 741, 725, 690, 750, 730, 690, 660, 540, 430,...
                 670, 630, 600, 590, 590, 600, 740, 800, 940, 960, 989, 1056, 1083, 1039, 1045, 930, 950, 978, 969, 932, 985, 940, 920, 730,...
                 450, 457.1, 464.3, 471.4, 478.6, 485.7, 520, 565, 700, 830, 960, 987, 976, 880, 897, 830, 941.4, 941.7, 850, 700, 650, 650, 575, 400,...
                 650, 666.7, 683.3, 600, 616.7, 633.3, 650, 700, 800, 850, 900, 893.3, 836.7, 880, 850, 870, 940, 966, 976.2, 886.5, 876.8, 777, 720, 750]'/Sbase;
E_beta = 0.9; % 碳捕集效率
delta_xz = 1;%烟气分流比限值
...
P_D1 = 10/Sbase;
% 风电预测功率
P_predict23 = [101.5354433, 109.754919, 107.6934759, 99.70100773, 105.1803393, 109.5938896, 116.6003918, 106.2809465, 96.55900751, 88.7920412, 70.82870473, 63.44550811, 49.66194391, 36.60327348, 34.37090338, 32.65712234, 35.79285476, 42.78481108, 50.33486619, 68.0972751, 84.89900022, 93.75642266, 97.15189551, 104.5288674,...
               86.67792994, 82.81971655, 86.11585946, 76.86116995, 79.42513169, 78.16894149, 77.3923782, 71.20348368, 57.15523866, 52.95728635, 50.41964931, 53.47104885, 47.31772919, 46.21625637, 41.49033991, 40.86198181, 47.16144843, 45.63485673, 48.19303447, 67.29210429, 74.57736409, 80.46775276, 83.76730972, 89.3236515,...
               169.0869932, 181.5567182, 183.4692342, 183.7001892, 171.0240362, 154.7376901, 142.3822899, 136.089448, 127.9917186, 96.19559884, 91.80399629, 80.7058997, 70.75475284, 69.63098785, 77.84693879, 76.82974219, 66.57168647, 65.7707681, 85.18873093, 97.35381507, 104.3651762, 114.6288012, 135.1586558, 159.4699069,...
               102.5546591, 102.8054795, 108.0881568, 101.0097939, 107.3092366, 118.563866, 116.9543222, 120.41058, 126.1275906, 123.0645029, 104.8953124, 78.61875886, 65.52009066, 45.78356584, 41.11059942, 35.47358765, 38.51018157, 39.55829838, 42.02310657, 43.18066311, 58.28306305, 74.54098017, 78.20137599, 88.83930604]'/Sbase;%该风场容量为390MW
P_predict27 = [266.0367593, 263.5092031, 265.7104348, 261.6380752, 265.6647769, 238.8958294, 258.3456898, 256.4707815, 199.4063396, 164.9607317, 158.397803, 140.1976845, 137.7615715, 148.9003129, 141.9807933, 151.3475427, 144.3794297, 148.874679, 172.1498061, 185.8389283, 214.8976196, 245.1756518, 248.3846512, 229.1140116,...
               323.3764011, 317.0672966, 290.7380682, 245.5905582, 246.0065491, 214.086545, 210.2052799, 208.9012573, 225.3374948, 212.4634211, 231.9425717, 254.5457145, 260.678996, 263.5199853, 247.3174173, 277.7094081, 293.9744066, 295.5504476, 282.4592318, 252.3895349, 235.8135765, 242.6796951, 279.7698785, 299.3098034,...
               367.472431, 356.9106357, 326.2150986, 308.4304066, 339.1413314, 326.6366373, 334.7795309, 342.2235065, 299.9933147, 246.4867918, 238.3075285, 252.4129447, 320.5171108, 344.4022794, 347.200607, 391.2038897, 387.6419588, 367.2238761, 378.5371925, 371.996488, 363.9935079, 351.012443, 361.6517508, 371.3930442,...
               164.5925684, 161.8065653, 159.0886511, 147.3812445, 146.2471562, 161.6236523, 168.8629218, 170.4721205, 182.205493, 204.7221299, 177.5438182, 163.0474545, 159.7465651, 135.7167871, 135.9053694, 135.1690911, 125.2841339, 136.4921162, 126.0656053, 136.3669099, 135.9978925, 146.9568736, 150.3926508, 172.8439097]'/Sbase;%该风场容量为1000MW
% 净负荷
...
u_ex = binvar(N,Hours,Years,'full');
...
v_ex = binvar(N,Hours,Years,'full');
...
w_ex = binvar(N,Hours,Years,'full');
%燃煤机组
...
toc
% % %test 
Cons = [Cons, I_trans_gexist == 0];
% Cons = [Cons, I_gen_ccs_1 == 0];
% Cons = [Cons, I_gen_ccs_2 == 0];
% Cons = [Cons, I_gen_ccs_3 == 0];
% Cons = [Cons, I_gen_ccs_4 == 0];
...
Cons = [Cons,u_ex(gen_c,:,:) == 0];
Cons = [Cons,v_ex(gen_c,:,:) == 0];
Cons = [Cons,w_ex(gen_c,:,:) == 0];
...
%         改造后用可变运行方式计算：
          Cons = [Cons, g_min_all(1) * u_ex(node, :, year) <= g_exist_c(node, :, year) <= g_max_all(1) * u_ex(node, :, year)];
    end
end
...
for y = 1:Years
    for h = 1:Hours
        ...
        Cons = [Cons, g_exist(23, h, y) ==  g_exist_w1_net(1,h,y)];
        Cons = [Cons, g_exist(27, h, y) ==  g_exist_w2_net(1,h,y)];
...
for y =1: Years
    for i = 1:3
        j = gen_node(i);
        Cons = [Cons,g_exist_c(j,:,y) == g_trans_net(i,:,y) + g_trans_ccs(i,:,y)* I_trans_gexist(i,y)];
        for t = 1:Hours
            Cons = [Cons,g_trans_ccs(i,t,y) == lamda_a * E_trans_ab(i,t,y) + lamda_dc *  E_trans_de(i,t,y) + P_D1 ];
            Cons = [Cons,E_trans_ab(i,t,y) <= ((g_trans_net(i,t,y) + g_trans_ccs(i,t,y))) * cei(1) * E_beta * I_trans_gexist(i,y)];
            Cons = [Cons, E_trans_ab(i,t,y) <= rate_max(1) * I_trans_gexist(i,y)];
            Cons = [Cons, E_trans_de(i,t,y) <= rate_max(1) * I_trans_gexist(i,y)];%流速设置为5000
        end
    end
end
Cons = [Cons,E_trans_ab >= 0];
...
for i=1:N_new
    ...
    for t = 1:Hours
        ...
        Cons = [Cons, v1(i,:,t,:) <= I_gen_coal_1(i,:,:)];
        Cons = [Cons, v2(i,:,t,:) <= I_gen_coal_2(i,:,:)];
        Cons = [Cons, w1(i,:,t,:) <= I_gen_coal_1(i,:,:)];
        Cons = [Cons, w2(i,:,t,:) <= I_gen_coal_2(i,:,:)];
        ...
        Cons = [Cons, vccs1(i,:,t,:) <= I_gen_ccs_1(i,:,:)];
        Cons = [Cons, vccs2(i,:,t,:) <= I_gen_ccs_2(i,:,:)];
        Cons = [Cons, wccs1(i,:,t,:) <= I_gen_ccs_1(i,:,:)];
        Cons = [Cons, wccs2(i,:,t,:) <= I_gen_ccs_2(i,:,:)];
        % 燃气机组发电功率上限 均可以启停
        ...
        Cons = [Cons, v3(i,:,t,:) <= I_gen_coal_1(i,:,:)];
        Cons = [Cons, v4(i,:,t,:) <= I_gen_coal_2(i,:,:)];
        Cons = [Cons, w3(i,:,t,:) <= I_gen_coal_1(i,:,:)];
        Cons = [Cons, w4(i,:,t,:) <= I_gen_coal_2(i,:,:)];
...
%         % 碳捕集机组能量和二氧化碳流动关系
        Cons = [Cons,energy_ccs1(i,:,t,:) == lamda_a * E_ccs_ab_1(i,:,t,:) + lamda_dc *  E_ccs_de_1(i,:,t,:) + P_D1*uccs1(i,:,t,:)];
        Cons = [Cons,energy_ccs2(i,:,t,:) == lamda_a * E_ccs_ab_2(i,:,t,:) + lamda_dc *  E_ccs_de_2(i,:,t,:) + P_D1*uccs2(i,:,t,:)];
...
Cons = [Cons,E_ccs_ab_1 <= rate_max(1) * uccs1];
Cons = [Cons,E_ccs_ab_2 <= rate_max(2) * uccs2];
Cons = [Cons,E_ccs_de_1 <= rate_max(1) * uccs1];
Cons = [Cons,E_ccs_de_2 <= rate_max(2) * uccs2];
for t= 1:Hours
...
for y = 1:Years
    ...
    Cons = [Cons,total_ccs_capacity_y(y) == sum(sum(I_gen_ccs_1(:,:,y),2))*(g_max_all(1)-P_D1) + ...
                                            ...
                                            sum(g_max_all(1)-P_D1) * sum(I_trans_gexist(:,y))];
...
for t = T_on_min : Hours
    ...
    Cons = [Cons, sum(v_ex(:,start_idx:t,:), 2) <= u_ex(:,t,:)];
end
for t = T_off_min : Hours
    ...
    Cons = [Cons, sum(w_ex(:,start_idx:t,:), 2) <= 1 - u_ex(:,t,:)];
end
for t = 2 : Hours
    ...
    Cons = [Cons,u_ex(:,t,:)-u_ex(:,t-1,:) == v_ex(:,t,:) - w_ex(:,t,:)];
end
toc
display('*** 机组启停 建立完成！***')
%% Obj: 计算投资成本
...
for year = 1:Years
    ...
    for t = year:Years
        %% 常规燃煤机组
        Obj_inv_coal = Obj_inv_coal + A_gen(1) / (1 + r)^((t - year) * period_length) * sum(sum(x_gen_coal_1(:, :, year)));
        Obj_inv_coal = Obj_inv_coal + A_gen(2) / (1 + r)^((t - year) * period_length) * sum(sum(x_gen_coal_2(:, :, year)));
        Obj_inv_coal = Obj_inv_coal + A_gen(3) / (1 + r)^((t - year) * period_length) * sum(sum(x_gen_coal_3(:, :, year)));
        Obj_inv_coal = Obj_inv_coal + A_gen(4) / (1 + r)^((t - year) * period_length) * sum(sum(x_gen_coal_4(:, :, year)));
        %% 碳捕集机组
        Obj_inv_ccs = Obj_inv_ccs + A_ccs(1) / (1 + r)^((t - year) * period_length) * sum(sum(x_gen_ccs_1(:, :, year)));
        Obj_inv_ccs = Obj_inv_ccs + A_ccs(2) / (1 + r)^((t - year) * period_length) * sum(sum(x_gen_ccs_2(:, :, year)));
        Obj_inv_ccs = Obj_inv_ccs + A_ccs(3) / (1 + r)^((t - year) * period_length) * sum(sum(x_gen_ccs_3(:, :, year)));
        Obj_inv_ccs = Obj_inv_ccs + A_ccs(4) / (1 + r)^((t - year) * period_length) * sum(sum(x_gen_ccs_4(:, :, year)));
        %% 燃气机组
        Obj_inv_gas = Obj_inv_gas + A_gas(1) / (1 + r)^((t - year) * period_length) * sum(sum(x_gen_gas_1(:, :, year)));
        Obj_inv_gas = Obj_inv_gas + A_gas(2) / (1 + r)^((t - year) * period_length) * sum(sum(x_gen_gas_2(:, :, year)));
        %% 改造
        Obj_inv_trans = Obj_inv_trans + A_gen_trans(1) / (1 + r)^((t - year) * period_length) * sum(x_trans_gexist(:,year));
    end
end
...
for t = 1:Hours
    for y = 1:Years
        Obj_ope_shed = Obj_ope_shed + (M * sum(pd_shed(:,t,y)) )*days_season*period_length;%切负荷成本 和 原有机组发电成本
        C_q1(t,y) = K_q * ((P_predict23(t) - g_exist_w1(1, t, y)) + (P_predict27(t) - g_exist_w2(1, t, y))); % 弃风惩罚成本
        for i = 1:length(gen_node)
            node = gen_node(i);
            Obj_ope_total = Obj_ope_total + (1 - I_trans_gexist(i, year))* cost(1).* g_exist_c(node,t,y)*days_season*3 + ...
                I_trans_gexist(i, year) * cost(1).* g_exist_c(node,t,y)*days_season*3;%原有燃煤机组考虑是否改造后的发电成本
        end
        for i = 1:4
            Obj_ope_total = Obj_ope_total + sum(sum(cost(i).*sum_type_g(:,i,t,y)))*days_season*3;%sum_type_g单位：100兆瓦时 cost单位：每100MW费用 总单位就是元
            Obj_ope_total = Obj_ope_total + sum(sum(cost(i).*sum_type_g_ccs(:,i,t,y)))*days_season*3;
        end
        for i = 1:2
            Obj_ope_total = Obj_ope_total + sum(sum(cost_gas(i).*sum_type_g_gas(:,i,t,y)))*days_season*3;
        end
    end
end

%启停成本
C_qiting(1)= sum(sum(sum(sum(v1 + w1)))) * C_start(1);%燃煤1启停
C_qiting(2)= sum(sum(sum(sum(v2 + w2)))) * C_start(2);%燃煤2启停
C_qiting(3)= sum(sum(sum(sum(v3 + w3)))) * C_start(3);%燃气1启停
C_qiting(4)= sum(sum(sum(sum(v4 + w4)))) * C_start(4);%燃气2启停
C_qiting(5)= sum(sum(sum(sum(vccs1 + wccs1)))) * C_start(1);%燃煤1启停
C_qiting(6)= sum(sum(sum(sum(vccs2 + wccs2)))) * C_start(2);%燃煤2启停
C_qiting(7)= sum(sum(sum(sum(v_ex + w_ex)))) * C_start(1);%燃煤2启停
Obj_C_qiting = sum(C_qiting)*days_season*3;
toc
display('**Part II 结束**')
...
for y = 1:Years
    %     灵活运行方式的碳成本这么计算：
    for i =1:3
        Cons = [Cons,cost_carbon_gexist(i,y) ==  carbon_tax(y) * sum((cei(1) -  carbon_quota(2,y)) * (g_trans_ccs(i,:,y) + g_trans_net(i,:,y)) - E_trans_ab(i,:,y))];
...
Obj_carbon_coal = sum(sum(cost_carbon_coal))* days_season*period_length;
Obj_carbon_ccs = sum(sum(cost_carbon_ccs))* days_season*period_length;
Obj_carbon_gas = sum(sum(cost_carbon_gas))* days_season*period_length;
Obj_carbon_gexist = sum(sum(cost_carbon_gexist))* days_season*period_length;
Obj_q = sum(sum(C_q1 * days_season*period_length));%弃风惩罚成本
...
Obj_sale = (sum(sum(sale_ccs))+sum(sum(sale_trans)))*days_season*period_length;
toc
display('**Part III 结束**')
...
s_g_exist_w = s_g_exist-s_g_exist_c;
s_g_trans_net = value(g_trans_net);
s_g_trans_ccs = value(g_trans_ccs);
...
s_Obj_carbon_gexist= value(Obj_carbon_gexist);
s_Obj_q=value(Obj_q);
s_Obj_ope_shed = value(Obj_ope_shed);
s_Obj_sale = value(Obj_sale);
s_Obj_C_qiting = value(Obj_C_qiting);
...
s_v_ex = value(v_ex);
s_w_ex = value(w_ex);
s_g_exist_toccs = value(g_exist_toccs);
s_sum_N_g_ccstoccs =  value(sum_N_g_ccstoccs);
%装机容量
...
for t = 1:Hours
    for y = 1:Years
        cost_ope(1,t,y) = sum(cost(1)*s_g_exist_c(:,t,y))*days_season*3;%原有燃煤机组发电成本
        for i = 1:4
            cost_ope(2,t,y) = cost_ope(2,t,y) + sum(sum(cost(i)*s_sum_type_g(:,i,t,y)))*days_season*period_length;
            cost_ope(3,t,y) = cost_ope(3,t,y) + sum(sum(cost(i)*s_sum_type_g_ccs(:,i,t,y)))*days_season*period_length;
        end
        for i = 1:2
            cost_ope(4,t,y) = cost_ope(4,t,y) + sum(sum(cost_gas(i).*s_sum_type_g_gas(:,i,t,y)))*days_season*period_length;
        end
    end
end
...
for y = 1:Years
    for i = 1:length(gen_node)
        node = gen_node(i);
        carbon_emission_gexist(i,y) = (sum(s_g_exist_c(node,:,y)) * ((1 - s_I_trans_gexist(i, y))* cei(1)  + s_I_trans_gexist(i, y) * cei_ccs(1)))*days_season*period_length;
        cost_carbon_gexist_years(i,y) = sum(s_g_exist_c(node,:,y)) * ((1 - s_I_trans_gexist(i, y))*cei(1)  + s_I_trans_gexist(i, y) * cei_ccs(1) - carbon_quota(1,y)) * carbon_tax(y)*days_season*period_length;%原有燃煤机组考虑是否改造后
    end
end
%总碳排
carbon_emission_coal = sum(sum(s_sum_coal .* cei'))*days_season*period_length;
carbon_emission_ccs = sum(sum(sum(sum(s_E_ccs_NET_1,2)+sum(s_E_ccs_NET_2,2)+sum(s_E_ccs_NET_3,2)+sum(s_E_ccs_NET_4,2))))*days_season*period_length;
carbon_emission_gas = sum(sum(s_sum_gas .* cei_gas'))*days_season*period_length;
carbon_emission = sum(sum(carbon_emission_gexist)) + carbon_emission_coal+carbon_emission_ccs+carbon_emission_gas;
...
% 创建按钮回调函数
set(year1Button, 'Callback', @(src, event) plotResults(1,Hours,cost_ope,s_sum_N_g,s_sum_N_g_ccs,s_sum_N_g_ccstoccs,s_g_exist_toccs,s_sum_N_g_gas,s_g_exist_c,s_g_exist_w,s_pd_shed,P_load,s_I_lines,s_x_gen_coal_1,s_x_gen_coal_2,s_x_gen_coal_3,s_x_gen_coal_4,s_x_gen_ccs_1,s_x_gen_ccs_2,s_x_gen_ccs_3,s_x_gen_ccs_4,s_x_gen_gas_1,s_x_gen_gas_2,I,J,l_E,N_new,gen_nodes_new));
set(year2Button, 'Callback', @(src, event) plotResults(2,Hours,cost_ope,s_sum_N_g,s_sum_N_g_ccs,s_sum_N_g_ccstoccs,s_g_exist_toccs,s_sum_N_g_gas,s_g_exist_c,s_g_exist_w,s_pd_shed,P_load,s_I_lines,s_x_gen_coal_1,s_x_gen_coal_2,s_x_gen_coal_3,s_x_gen_coal_4,s_x_gen_ccs_1,s_x_gen_ccs_2,s_x_gen_ccs_3,s_x_gen_ccs_4,s_x_gen_gas_1,s_x_gen_gas_2,I,J,l_E,N_new,gen_nodes_new));
set(year3Button, 'Callback', @(src, event) plotResults(3,Hours,cost_ope,s_sum_N_g,s_sum_N_g_ccs,s_sum_N_g_ccstoccs,s_g_exist_toccs,s_sum_N_g_gas,s_g_exist_c,s_g_exist_w,s_pd_shed,P_load,s_I_lines,s_x_gen_coal_1,s_x_gen_coal_2,s_x_gen_coal_3,s_x_gen_coal_4,s_x_gen_ccs_1,s_x_gen_ccs_2,s_x_gen_ccs_3,s_x_gen_ccs_4,s_x_gen_gas_1,s_x_gen_gas_2,I,J,l_E,N_new,gen_nodes_new));
set(year4Button, 'Callback', @(src, event) plotResults(4,Hours,cost_ope,s_sum_N_g,s_sum_N_g_ccs,s_sum_N_g_ccstoccs,s_g_exist_toccs,s_sum_N_g_gas,s_g_exist_c,s_g_exist_w,s_pd_shed,P_load,s_I_lines,s_x_gen_coal_1,s_x_gen_coal_2,s_x_gen_coal_3,s_x_gen_coal_4,s_x_gen_ccs_1,s_x_gen_ccs_2,s_x_gen_ccs_3,s_x_gen_ccs_4,s_x_gen_gas_1,s_x_gen_gas_2,I,J,l_E,N_new,gen_nodes_new));
set(year5Button, 'Callback', @(src, event) plotResults(5,Hours,cost_ope,s_sum_N_g,s_sum_N_g_ccs,s_sum_N_g_ccstoccs,s_g_exist_toccs,s_sum_N_g_gas,s_g_exist_c,s_g_exist_w,s_pd_shed,P_load,s_I_lines,s_x_gen_coal_1,s_x_gen_coal_2,s_x_gen_coal_3,s_x_gen_coal_4,s_x_gen_ccs_1,s_x_gen_ccs_2,s_x_gen_ccs_3,s_x_gen_ccs_4,s_x_gen_gas_1,s_x_gen_gas_2,I,J,l_E,N_new,gen_nodes_new));