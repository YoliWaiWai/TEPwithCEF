% 创建绘图函数
function plotResults(year,Hours,cost_ope,s_sum_N_g,s_sum_N_g_ccs,s_sum_N_g_ccstoccs,s_g_exist_toccs,s_sum_N_g_gas,s_g_exist_c,s_g_exist_w,s_pd_shed,P_load,s_I_lines,s_x_gen_coal_1,s_x_gen_coal_2,s_x_gen_coal_3,s_x_gen_coal_4,s_x_gen_ccs_1,s_x_gen_ccs_2,s_x_gen_ccs_3,s_x_gen_ccs_4,s_x_gen_gas_1,s_x_gen_gas_2,I,J,l_E,N_new,gen_nodes_new)
    N = size(P_load, 1);
    gen_nodes_new = [9,11,13,21,22,25,27];
% 根据选择的年份绘制相应的图形
    subplot(2, 2, 1); % Change subplot to 2 rows and 2 columns
    % Plot yearly planning results
    sum_type_g_all = sum(s_sum_N_g, 1);
    sum_type_g_all_ccs = sum(s_sum_N_g_ccs, 1);
    sum_ccs_to_ccs =sum(s_sum_N_g_ccstoccs, 1);
    sum_g_exist_toccs =sum(s_g_exist_toccs, 1); 
    sum_type_g_all_gas = sum(s_sum_N_g_gas, 1);
    sum_g_exist_w = sum(s_g_exist_w, 1);
    sum_g_exist_c = sum(s_g_exist_c, 1);
    sum_s_pd_shed = sum(s_pd_shed);
    sum_type_g_all = [sum_g_exist_w;sum_g_exist_c; sum_type_g_all;sum_type_g_all_gas;sum_type_g_all_ccs;sum_s_pd_shed;sum_ccs_to_ccs+sum_g_exist_toccs];%gas
    ...
    xlabel('时段/h');
    ylabel('机组出力/100MW');
    legend('风电机组净出力','原燃煤机组净出力','新建燃煤机组','燃气机组','碳捕集机组净出力','失负荷','碳捕集系统能耗','负荷');
    ...
    for i = 1:N_new
          j =  gen_nodes_new(i);
            ...
            text(h2.XData(j)+0.1, h2.YData(j)-0.1, label_str, 'HorizontalAlignment', 'right', 'VerticalAlignment', 'top', 'Color', 'r', 'FontSize', 8);
            ...
            label_str = sprintf('1型: %d\n2型: %d\n3型: %d\n4型: %d', num_gen1, num_gen2, num_gen3, num_gen4);
            text(h2.XData(j)+1.6, h2.YData(j)-0.1, label_str, 'HorizontalAlignment', 'right', 'VerticalAlignment', 'top', 'Color', 'g', 'FontSize', 8);
            num_gen1 = sum(s_x_gen_gas_1(i, :, year));   
            num_gen2 = sum(s_x_gen_gas_2(i, :, year)); 
            label_str = sprintf('1型: %d\n2型: %d\n3型: %d\n4型: %d', num_gen1, num_gen2);
            text(h2.XData(j)+1.6, h2.YData(j)-0.1, label_str, 'HorizontalAlignment', 'right', 'VerticalAlignment', 'top', 'Color', 'b', 'FontSize', 8);
    end
    if year == 5
 % 如果是第五年，单独绘制第五年的运行结果
     figure('Name', '第五年单独运行结果');
     % 绘制第五年的运行结果
     sum_type_g_all = sum(s_sum_N_g, 1);
     sum_type_g_all_ccs = sum(s_sum_N_g_ccs, 1);
     sum_type_g_all_gas = sum(s_sum_N_g_gas, 1);
     ...
     sum_ccs_to_ccs =sum(s_sum_N_g_ccstoccs, 1);
    sum_g_exist_toccs =sum(s_g_exist_toccs, 1); 
     sum_type_g_all = [sum_g_exist_w; sum_g_exist_c; sum_type_g_all; sum_type_g_all_ccs; sum_type_g_all_gas; sum_s_pd_shed;sum_ccs_to_ccs+sum_g_exist_toccs]; % gas
     sum_type_g_all = sum_type_g_all(:,:,year)';
     h = bar(1:Hours, sum_type_g_all, 'stacked');
     hold on;
     pp = sum(P_load, 1);
     ...
     xlabel('时段/h');
     ylabel('机组出力/100MW');
     legend('风电机组净出力','原燃煤机组净出力','新建燃煤机组净出力','碳捕集机组净出力','燃气机组净出力','失负荷','碳捕集系统能耗','负荷');
     title(['第', num2str(year), '个规划期机组出力情况']);
     max_load = max(P_load(:));
     ylim([0, max_load*12]);
    end
