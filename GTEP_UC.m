...
for t = 1:T
    ...
    Obj_u=Obj_u+Cu_NL.*u(i,:,t);
    Obj_up=Obj_up+Cup.*v(i,:,t);
    Obj_down=Obj_down+Cdown.*w(i,:,t);
    end
    Obj = Obj+Obj_inv+Obj_ope+Obj_u+Obj_up+Obj_down;
%% Cons1: 直流潮流约束+TEP
Cons_DC=[Cons,
    % In*p==g-d,In'*theta==p.*X,theta(1)==0
    -(1-x_lines)*M <= In'*theta(:,t) - p(:,t).* xb <= (1-x_lines)*M,
    sum_N_g == sum(g,2),
    In*p(:,t) == sum_N_g(:,:,t)+g_exist(:,t)-(P_load(:,t)-pd_shed(:,t)),
    theta(1,t)==0,
    -x_lines.*p_max <= p(:,t) <=x_lines.*p_max,
    pd_shed(:,t)>=0;
];
Cons=[Cons, Cons_DC];
%% Cons2: 机组发电功率约束
