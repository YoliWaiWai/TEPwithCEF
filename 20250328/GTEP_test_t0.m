...
carbon_tax =     [95   105   115   125   135   145   155   165];         % 碳税﻿ 单位：元/tCO2
% carbon_tax =carbon_tax*0.8;
...
Carbon_dioxide_price = 200;%出售二氧化碳价格
%Carbon_dioxide_price = 0;%出售二氧化碳价格
%生成一组24h负荷需求数据
...
P_D1 = 10/Sbase;
% 风电预测功率
P_predict23 = [320, 250, 310, 270, 340, 280, 150, 140, 200, 190, 110, 70,50, 100, 130, 145, 155, 210, 290, 310, 330, 350, 360, 340]'*3/4/Sbase;%该风场容量为300MW
...
toc
% %test 用于测试为什么不运行燃气
Cons = [Cons, sum(sum(sum(sum(u3))))+sum(sum(sum(sum(u4)))) >= 1];
...
for t = 1:Hours
    for y = 1:Years
        ...
        for i = 1:length(gen_node)
            node = gen_node(i);
            Obj_ope_total = Obj_ope_total + (1 - I_trans_gexist(i, year))* cost(1).* g_exist_c(node,t,y)*365*period_length + ...
                I_trans_gexist(i, year) * cost(1).* g_exist_c(node,t,y)*365*period_length;%原有燃煤机组考虑是否改造后的发电成本
        end
        for i = 1:4
            Obj_ope_total = Obj_ope_total + sum(sum(cost(i).*sum_type_g(:,i,t,y)))*365*period_length;%sum_type_g单位：100兆瓦时 cost单位：每100MW费用 总单位就是元
            Obj_ope_total = Obj_ope_total + sum(sum(cost(i).*sum_type_g_ccs(:,i,t,y)))*365*period_length;
        end
        for i = 1:2
            Obj_ope_total = Obj_ope_total + sum(sum(cost_gas(i).*sum_type_g_gas(:,i,t,y)))*365*period_length;
        end
    end
end
