...
g_exist = sdpvar(N,1);
x_lines = binvar(L,1);
x_gen_coal = binvar(3,K);%在node【1，2，3】是否建设k型机组
...
Obj_ope=M*sum(pd_shed)+sum(c2.*g_exist(1:3).*2+c1.*g_exist(1:3));%原有机组发电成本
%新增机组发电成本
for i=1:3
...
Cons_DC=[
    ...
    In*p == sum(g,2)+g_exist-(pd-pd_shed);
    theta(1)==0,
...
%% Cons2: 机组发电功率约束
% 原有机组
Cons=[Cons, g_exist(2:6)==0];
Cons=[Cons,0 <= g_exist(1); g_exist(1)<= g_max(1)];
% 新增机组
for i=1:3
    Cons=[Cons, sum(x_gen_coal(i,:))<=1];%每个节点最多建设一种
    for k=1:K
        Cons=[Cons, 0 <= g(i,k) <= x_gen_coal(i,k).*g_max_all(k)];%机组发电功率上限
    end
end
Cons=[Cons, g([1,4:6],:) == 0];%只在2、3节点建设机组
...
for i= 4:6
    label_str = sprintf('负荷需求：%.2f',pd(i)); 
    text(h2.XData(i)-0.4, h2.YData(i)+0.2, label_str, ...
        'HorizontalAlignment', 'left', 'VerticalAlignment', 'middle', 'Color', 'b', 'FontSize', 8);
end